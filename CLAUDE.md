...
System dependencies (Arch package names): `qt6-base`, `qt6-declarative` (Qml + Quick), `qt6-quickcontrols2` (provided by qt6-declarative), `kirigami` (KF6, ≥ 6.x), `qqc2-desktop-style` (so Qt Quick Controls render through QStyle), `oxygen` (Qt 6 Oxygen QStyle plugin — fallback chain Oxygen → Breeze → Fusion), `mpv` (libmpv via pkg-config), `cmake`, `ninja`. Qt's AUTOMOC/AUTOUIC/AUTORCC are on, so MOC files are generated under `build/jellycute_autogen/` automatically — no manual `moc` invocations.
...
- **`main.cpp`** — bootstraps Qt, requests an OpenGL 3.3 Compatibility surface format *before* `QApplication` (libmpv's render API needs ≥ 3.x), pins Qt Quick to the OpenGL backend (`QQuickWindow::setGraphicsApi(OpenGL)`) since `QQuickFramebufferObject` doesn't work on RHI/Vulkan, sets `QQuickStyle` to `org.kde.desktop` so QtQuick Controls render through QStyle, forces `LC_NUMERIC=C` *after* `QApplication` (Qt resets it; libmpv's option parser requires C locale), picks the QStyle backend (Oxygen → Breeze → Fusion), installs the carbon QPalette so the QStyle gradients render in dark greys, restores the persisted active account into `JellyfinClient`, registers QML types, exposes the context properties, and loads `qrc:/qml/Main.qml`.
